  ...
  useEffect(() => {
    ...
    peer.on('error', (err: any) => {
      if (err.type === 'unavailable-id') {
        console.warn('Peer ID already taken. This usually happens after a quick refresh. The old connection should timeout soon.');
      } else if (err.type === 'peer-unavailable') {
        console.error('Peer not found:', err);
        alert("المستخدم غير متاح حالياً للاتصال. قد يكون غير متصل بالإنترنت.");
        endCall();
